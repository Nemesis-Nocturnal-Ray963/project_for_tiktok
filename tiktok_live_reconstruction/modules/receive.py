...
# 特定コメントのキーワード
POINT_TRIGGER_COMMENT = ["3pt", "参加", "ポイント"]
COMMENT_COOLDOWN_SECONDS = 300  # 5分
...
async def on_comment_mod(event,streamer_ID):
    global POINT_TOTAL,COMMENT_COOLDOWN,IS_BATTLE
    
    user_id = event.user.unique_id
    comment = event.comment.lower()
    now_ts = datetime.now().timestamp()
    if any(key in comment for key in POINT_TRIGGER_COMMENT):

        # --- 個人クールタイムチェック ---
        last = COMMENT_COOLDOWN.get(user_id, 0)
        if now_ts - last >= COMMENT_COOLDOWN_SECONDS:
            # ポイント付与
            COMMENT_COOLDOWN[user_id] = now_ts
            await point_queue.put(3)
            print(f"[POINT] +3pt comment by {event.user.nickname}")
            await update_point_display()
        else:
            print(f"[POINT] {event.user.nickname} is on cooldown")

    # 元のログ出力
    now = datetime.now()
    print(f"{event.user.nickname} >> {event.comment} at {now.strftime('%Y-%m-%d %H:%M:%S')} ")
...
async def on_battle_start():
    ...
    global IS_BATTLE
    IS_BATTLE = True
    print("[POINT] battle start → point x2")
...
async def on_battle_end():
    ...
    global IS_BATTLE
    IS_BATTLE = False
    print("[POINT] battle end → point x1")
