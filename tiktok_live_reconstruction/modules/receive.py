...
import queue
import os
print("2025年10月21日 ギフトギャラリー+α+イベント")
...
async def on_gift_mod(event,streamer_ID):
    ...
    if event.gift.streakable and not event.streaking:
        ...
        #     asyncio.create_task(summon_zombies(user, times,minecraft_id))
            ...
            asyncio.create_task(arcade_send_queue(("spawn_fireworks", [10, 0.3])))
        ...
        #     asyncio.create_task(mishka_storm(user,times,minecraft_id))
            ...
            for _ in range(count):
                ...
                asyncio.create_task(arcade_send_queue(("spawn_fireworks", [400, 300, (255, 200, 100)])))
                await asyncio.sleep(0.1)
