...
async def summon_glass_prison_near(escape_player: str, pursuance_player: str, radius=100, size=5, duration=5):
    ...
    # === 1. 逃走者の座標取得 ===
    pos = await get_player_pos(escape_player)
    if not pos:
        await lightning_fail_event(escape_player, pursuance_player)
        return
    cx, cy, cz = coords
...
async def lightning_fail_event(p1: str, p2: str):
    """座標取得失敗時のペナルティイベント"""
    cmds = [
        f"execute at {p1} run summon lightning_bolt ~ ~ ~",
        f"execute at {p2} run summon lightning_bolt ~ ~ ~",
        f"title {p1} title {{\"text\":\"§c転送失敗！\"}}",
        f"title {p2} title {{\"text\":\"§c転送失敗！\"}}",
        f"effect give {p1} instant_health 10 10",
        f"effect give {p2} instant_health 10 10",
        f"effect give {p1} speed 60 2 true",
        f"effect give {p2} speed 60 2 true",
    ]
    for cmd in cmds:
        await command_send_queue(cmd)
        await asyncio.sleep(0.05)  # レート制御
    ...
    def _query_pos():
        with MCRcon("127.0.0.1", "3699", port=25575) as mcr:
            for _ in range(16):
                ...
                if resp:
                    return resp
                time.sleep(0.1)  # スレッド内なのでOK
        return None

    # スレッドでRCON実行（非同期ループを止めない）
    resp = await asyncio.to_thread(_query_pos)

    if not resp:
        print(f"[WARN] 座標取得失敗: {player}")
        return None

    try:
        coords = [float(x.replace("d", "")) for x in resp.split('[')[1].split(']')[0].split(',')]
        print(f"[INFO] {player} -> {coords}")
        return coords
    except Exception as e:
        print(f"[ERROR] 座標解析失敗: {e}")
        return None



async def light_teleport_toward(escape_player: str, pursuance_player: str, distance: float = 20):
    """追跡者を逃走者方向に distance 分テレポート"""
    pos_a = await get_player_pos(escape_player)
    pos_b = await get_player_pos(pursuance_player)

    if not pos_a or not pos_b:
        print("[WARN] 座標取得失敗。テレポート中止。")
        await lightning_fail_event(escape_player, pursuance_player)
        return
    ...
    if length == 0:
        print("[INFO] 両者同位置のためスキップ。")
        return

    ux, uy, uz = dx / length, dy / length, dz / length
    new_x, new_y, new_z = bx + ux * distance, by + uy * distance, bz + uz * distance

    await command_send_queue(
        f"execute as {pursuance_player} at {pursuance_player} run tp {pursuance_player} {new_x:.2f} {new_y:.2f} {new_z:.2f}"
    )
